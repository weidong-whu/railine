...
\begin{figure}[!ht]
    ...
    \draw [fill=gray!5] (-7.7,20.5) rectangle  
    node {\large
    \begin{minipage}{5cm}
    Initial estimation of $\mathbf {\hat{x}}$ and $\mathrm {P}$
    ~(\cref{sec_seedgeneration})
    \end{minipage}
    } (-1.72,18.5);
    
    % 左侧矩形 Check for termination，左移 0.5 单位
    \draw [fill=gray!5] (-7.7,23.5) rectangle  
    node {\large
    \hspace{0.4em} 
    ...
    \caption{The flow of kalman filter for the \textit{RLP} estimation.}
...
\subsection{Railway line reconstruction with multiple images}
\label{sec_linereconstruction}

\subsection{The seed generation for railway track}
\label{sec_seedgeneration}
